...
class ModInfo(FileInfo):
    ...
    def mod_bsas(self, bsa_infos=None):
        ...
        if bush.game.fsName == u'Morrowind':
            ...
            return []
        elif bush.game.fsName in (u'Enderal', u'Skyrim'):
            # Skyrim only accepts the base name
            extra_bsas = u''
        elif bush.game.fsName == u'Skyrim Special Edition':
            # Skyrim SE accepts the base name and ' - Textures'
            extra_bsas = r'(?: \- Textures)?'
        else:
            # All other games accept arbitrary BSA names
            extra_bsas =  r'(?: \- \w+)?'
        bsa_pattern = (re.escape(self._modname) + extra_bsas +
