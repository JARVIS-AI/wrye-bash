...
def _cosave_encode(byte_str): return encode(byte_str,
...
class _xSEChunkARVR(_xSEChunk, _Dumpable):
    ...
    class _xSEEntryARVR(_ChunkEntry, _Dumpable):
        ...
        the standard _ChunkEntry methods and instead have to create out own,
        ...
        'raise AbstractException' statements to signal this fact."""
    ...
    def chunk_length(self):
        ...
        for element in self.elements:
            total_len += element.arvr_entry_length(self.key_type)
        return total_len
...
class _xSEChunkPLGN(_xSEChunk, _Dumpable, _Remappable):
    ...
    def chunk_length(self):
        total_len = 2
        for mod_entry in self.mod_entries:
            total_len += mod_entry.entry_length()
        return total_len
...
# TODO(inf) This is pretty hideous and could use some refactoring
# TODO(inf) Additionally, post 308 we may want to reorganize these into a
# records.py-like, game-specific structure
_xse_class_dict = {
...
def _get_xse_chunk_type(ins):
    """
    Reads a 4-byte string from the specified input stream and returns it,
    alongside a matching xSE chunk class for that string. If no matching class
    is found, the generic _xSEChunk class is returned instead.

    :param ins: The input stream to read from.
    :return: A tuple containing the read chunk type and a matching chunk class,
             or the generic one if no matching class was found.
    """
    # The chunk type strings are reversed in the cosaves
    ch_type = _cosave_decode(unpack_4s(ins))[::-1]
    return ch_type, _xse_class_dict.get(ch_type, _xSEChunk)
...
class _xSEPluginChunk(_AChunk, _Remappable):
    ...
    def __init__(self, ins):
        ...
        for x in xrange(num_chunks):
            ch_type, ch_class = _get_xse_chunk_type(ins)
            # If ch_type is None, that means we don't have to pass it on
            if ch_type:
                new_chunk = ch_class(ins, ch_type)
            else:
                new_chunk = ch_class(ins)
...
class _ACosave(_Dumpable, _Remappable):
    ...
    def _read_cosave(self):
        ...
        skipping to end to skip 12 bytes - otherwise, reading it is impossible.
        """
        with self.cosave_path.open('rb') as ins:
            self._read_cosave_header(ins)
            self._read_cosave_body(ins)
    ...
    def _read_cosave_body(self, ins):
        """
    ...
    def add_cosave_chunk(self, cosave_chunk):
        """
...
class xSECosave(_ACosave):
    ...
    def _read_cosave_body(self, ins):
        ...
        for x in xrange(my_header.num_plugin_chunks):
            self.add_cosave_chunk(_xSEPluginChunk(ins))
    ...
    def get_master_list(self):
        ...
        if first_chunk.chunk_type == u'PLGN':
            ...
            mod_list = first_chunk.mod_names
            # The second chunk may be a LIMD or LMOD chunk (e.g. on older
            # SKSE64 or F4SE versions)
            second_chunk = xse_chunks[1]
            if second_chunk.chunk_type in (u'LIMD', u'LMOD'):
                mod_list.extend(second_chunk.mod_names)
            return mod_list
...
class PluggyCosave(_ACosave):
    ...
    def _read_cosave(self):
        ...
        with self.cosave_path.open('rb') as ins:
            ...
            if actual_position != expected_position:
                raise FileError(self.cosave_path.tail, u'Invalid cosave: End '
                                                       u'control position was '
                                                       u'incorrect (expected '
                                                       u'%u, but got %u).' %
        ...
        if actual_crc != expected_crc:
            raise FileError(self.cosave_path.tail, u'Invalid cosave: Checksum '
                                                   u'does not match (expected '
                                                   u'%X, but got %X).' %
    ...
    def _read_cosave_body(self, ins):
        ...
        while raw_type:
            ...
            self.add_cosave_chunk(block_type(ins, record_type))
            raw_type = ins.read(1)
